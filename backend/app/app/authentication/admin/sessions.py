...
def create_admin_session(admin_id: int, request, remember_me: bool):
    ...
    with get_conn() as conn:
        conn.execute("""
            ...
            ("sessionId", "adminId", "refreshTokenHash", "deviceName", browser, os, "ipAddress", "createdAt", "lastActivity", "expiresAt", "rememberMe")
...
def get_admin_session_db(session_id: str):
    with get_conn() as conn:
        return conn.execute("SELECT * FROM \"adminSessions\" WHERE \"sessionId\" = %s AND status = 'Active'", (session_id,)).fetchone()

def revoke_admin_session_db(session_id: str):
    now = datetime.utcnow().isoformat()
    with get_conn() as conn:
        conn.execute("UPDATE \"adminSessions\" SET status = 'Revoked', \"revokedAt\" = %s WHERE \"sessionId\" = %s", (now, session_id))
        conn.commit()
