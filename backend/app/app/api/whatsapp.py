...
async def send_whatsapp_single(landlordUuid: str, request: Request, tenantId: int, billNo: str, principal=Depends(get_current_landlord_api_strict)):
    ...
    link = f"{public_app_url()}/tenant/{quote(landlordUuid)}/QR/{getattr(tenant, 'propertyId', 0) or 0}/{tenant.id}/{quote(token)}"
    ...
    try:
        ...
        with get_conn() as conn:
            row = conn.execute(
                'SELECT "encryptedPin" FROM "tenantPinAdminStore" WHERE "tenantId" = %s',
                (tenant.id,)
            ).fetchone()
        if row:
            tenant_portal_pin = decrypt_admin_view_pin(row["encryptedPin"])
