...
def _resolve_tenant_property_id(tenant_id: int, landlord_id):
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT id FROM \"landlordProperties\" WHERE \"landlordId\" = %s ORDER BY \"sortOrder\", id LIMIT 1",
            (landlord_id,),
        ).fetchone()
        if row:
            return row["id"]
...
async def public_tenant_login(propertyId: int, tenantId: int, viewToken: str, request: Request, response: Response, login_req: EncryptedLoginRequest):
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT \"failedAttempts\", \"lockedUntil\" FROM tenants WHERE id = %s",
            (tenant.id,),
        ).fetchone()
    ...
    def _record_failed_attempt(detail: str):
        failed_attempts = (row["failedAttempts"] or 0) + 1 if row else 1
        locked_until_str = None
        if failed_attempts >= 5:
            locked_until_str = (datetime.utcnow() + timedelta(minutes=15)).isoformat()
        with get_conn() as conn:
            conn.execute(
                "UPDATE tenants SET \"failedAttempts\" = %s, \"lockedUntil\" = %s WHERE id = %s",
    ...
    if (row and row["failedAttempts"]) or (row and row["lockedUntil"]):
        with get_conn() as conn:
            conn.execute(
                "UPDATE tenants SET \"failedAttempts\" = 0, \"lockedUntil\" = NULL WHERE id = %s",
                (tenant.id,),
            )
            conn.commit()
...
async def global_tenant_login(request: Request, response: Response, login_req: EncryptedLoginRequest):
    ...
    with get_conn() as conn:
        row = conn.execute(
            'SELECT id, name, \"landlordId\", "viewToken", \"passwordHash\", \"propertyId\", '
            "\"passwordFailedAttempts\", \"passwordLockedUntil\", \"passwordResetRequired\" "
            "FROM tenants WHERE LOWER(\"tenantUsername\") = %s ORDER BY id LIMIT 1",
            (username,),
        ).fetchone()
    ...
    from datetime import datetime, timedelta
    if row["passwordLockedUntil"]:
        try:
            locked_until = datetime.fromisoformat(row["passwordLockedUntil"])
            if datetime.utcnow() < locked_until:
                raise HTTPException(status_code=429, detail="Account locked. Try again later.")
        except ValueError:
            pass

    if not verify_pin(password, row["passwordHash"]):
        log_audit(row["id"], "Portal Login Failed - Wrong Password", ip)
        failed_attempts = (row["passwordFailedAttempts"] or 0) + 1
        locked_until_str = None
        if failed_attempts >= 5:
            locked_until_str = (datetime.utcnow() + timedelta(minutes=15)).isoformat()
        with get_conn() as conn:
            conn.execute(
                "UPDATE tenants SET \"passwordFailedAttempts\" = %s, \"passwordLockedUntil\" = %s WHERE id = %s",
                (failed_attempts, locked_until_str, row["id"]),
            )
            conn.commit()
        raise HTTPException(status_code=401, detail="Invalid username or password")

    if (row["passwordFailedAttempts"] or 0) > 0:
        with get_conn() as conn:
            conn.execute(
                "UPDATE tenants SET \"passwordFailedAttempts\" = 0, \"passwordLockedUntil\" = NULL WHERE id = %s",
                (row["id"],),
            )
            conn.commit()
    ...
    with get_conn() as conn:
        landlord = conn.execute(
            "SELECT \"landlordUuid\" FROM \"landlordAccounts\" WHERE id = %s",
            (row["landlordId"],),
        ).fetchone()

    if not landlord:
        landlord = conn.execute(
            "SELECT \"landlordUuid\" FROM \"landlordAccounts\" ORDER BY id LIMIT 1"
        ).fetchone()

    if not landlord:
        raise HTTPException(status_code=500, detail="Landlord account not found")

    landlord_uuid = landlord["landlordUuid"]
    view_token = row["viewToken"]
    tenant_id = row["id"]
    property_id = row["propertyId"] or _resolve_tenant_property_id(tenant_id, row["landlordId"])
...
async def global_tenant_forgot_password(request: Request, login_req: EncryptedLoginRequest):
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT id FROM tenants WHERE LOWER(\"tenantUsername\") = %s ORDER BY id LIMIT 1",
            (username,),
        ).fetchone()
    ...
    if row:
        ...
        with get_conn() as conn:
            conn.execute(
                "UPDATE tenants SET \"passwordResetTokenHash\" = %s, \"passwordResetExpiresAt\" = %s, \"passwordResetRequestedAt\" = %s WHERE id = %s",
                (hash_pin(token), expires_at.isoformat(), now.isoformat(), row["id"]),
            )
            conn.execute(
                '''INSERT INTO \"tenantPasswordResetEvents\" ("tenantId", channel, \"tokenHash\", \"createdAt\", \"expiresAt\", \"requestedIp\") VALUES (%s, 'self-service', %s, %s, %s, %s)''',
...
async def global_tenant_reset_password(request: Request, response: Response, login_req: EncryptedLoginRequest):
    ...
    with get_conn() as conn:
        row = conn.execute(
            'SELECT id, name, \"landlordId\", "viewToken", \"passwordResetTokenHash\", \"passwordResetExpiresAt\" FROM tenants WHERE LOWER(\"tenantUsername\") = %s ORDER BY id LIMIT 1',
            (username,),
        ).fetchone()

    if not row or not row["passwordResetTokenHash"]:
        raise HTTPException(status_code=400, detail="Invalid or expired reset token")

    if not verify_pin(token, row["passwordResetTokenHash"]):
        raise HTTPException(status_code=400, detail="Invalid or expired reset token")

    if row["passwordResetExpiresAt"]:
        try:
            expires_at = datetime.fromisoformat(row["passwordResetExpiresAt"])
    ...
    with get_conn() as conn:
        conn.execute(
            "UPDATE tenants SET \"passwordHash\" = %s, \"passwordResetTokenHash\" = NULL, \"passwordResetExpiresAt\" = NULL, \"passwordResetRequestedAt\" = NULL, \"passwordResetRequired\" = 0, \"passwordFailedAttempts\" = 0, \"passwordLockedUntil\" = NULL, \"lastPasswordChangeAt\" = %s WHERE id = %s",
            (hash_pin(new_password), now.isoformat(), tenant_id),
        )
        conn.execute(
            '''INSERT INTO \"tenantPasswordHistory\" ("tenantId", \"passwordHash\", \"changedAt\", \"changedBy\") VALUES (%s, %s, %s, 'tenant-reset')''',
            (tenant_id, hash_pin(new_password), now.isoformat()),
        )
        conn.execute(
            'UPDATE \"tenantPasswordResetEvents\" SET \"usedAt\" = %s WHERE "tenantId" = %s AND \"usedAt\" IS NULL',
...
async def global_tenant_change_password(request: Request, response: Response, login_req: EncryptedLoginRequest):
    ...
    with get_conn() as conn:
        row = conn.execute(
            "SELECT id, \"passwordHash\" FROM tenants WHERE LOWER(\"tenantUsername\") = %s ORDER BY id LIMIT 1",
            (username,),
        ).fetchone()

    if not row or not row["passwordHash"] or not verify_pin(current_password, row["passwordHash"]):
    ...
    with get_conn() as conn:
        conn.execute(
            "UPDATE tenants SET \"passwordHash\" = %s, \"passwordResetRequired\" = 0, \"passwordResetTokenHash\" = NULL, \"passwordResetExpiresAt\" = NULL, \"passwordResetRequestedAt\" = NULL, \"passwordFailedAttempts\" = 0, \"passwordLockedUntil\" = NULL, \"lastPasswordChangeAt\" = %s WHERE id = %s",
            (hash_pin(new_password), now.isoformat(), tenant_id),
        )
        conn.execute(
            '''INSERT INTO \"tenantPasswordHistory\" ("tenantId", \"passwordHash\", \"changedAt\", \"changedBy\") VALUES (%s, %s, %s, 'tenant-change')''',
            (tenant_id, hash_pin(new_password), now.isoformat()),
        )
        conn.commit()
...
async def tenant_audit_logs(
    ...
    query = '''
        SELECT
            ...
            NULL AS target_type,
            NULL AS target_id,
            tl."ipAddress" AS ip_address,
            tl."metaJson" AS meta_json,
    ...
    if search:
        query += " AND (tl.action LIKE %s OR tl.\"ipAddress\" LIKE %s)"
        params.extend([f"%{search}%"] * 2)
    if date_from:
        query += " AND tl.\"createdAt\" >= %s"
    ...
    query += " ORDER BY tl.\"createdAt\" DESC LIMIT %s OFFSET %s"
...
async def submit_tenant_feedback(
    ...
    with _get_conn() as conn:
        row = conn.execute(
            """
            INSERT INTO "tenantQrFeedback"
            ("tenantId", "landlordId", "propertyId", "tenantName", "viewToken", "qrKey",
             message, "diagnosticsJson", "failedAttempts", status, "createdAt", "ipAddress")
