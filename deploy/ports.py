...
These are the ONLY port numbers that matter. Every layer derives from here:
  ...
  - compose.dev.yml         (backend 28002, storage 28003,
                             database 28004, nginx 28005;
                             frontend 28001 retired; ngrok service removed)
  - compose.prod.yml        (backend 28011, storage 28012, database 28013,
                             nginx 28014)
  - nginx listen/upstreams  (dev: gateway listen 28005 -> backend 28002;
                             prod: edge listen 28014 -> backend 28011)
...
DEV_PORTS = {
    "frontend": 28001,          # RETIRED — tenant-app Vite dev server removed from the
                                # dev stack (dev frontend hosts on Cloudflare Pages `dev`
                                # branch). Port kept so ports.env rendering stays stable.
    ...
    "nginx": 28005,             # dev edge gateway, host-published (ngrok tunnel target)
...
PORT_ENV_KEYS = [
    # (key, env, role)
    ("FRONTEND_DEV_PORT", "dev", "frontend"),  # retired (see DEV_PORTS) — kept for ports.env stability
